    ...
    common::AocResultType run() override {
        ...
        for (auto it_1 = std::ranges::begin(all_tiles); it_1 != std::ranges::end(all_tiles); ++it_1) {
            for (auto it_2 = std::next(it_1); it_2 != std::ranges::end(all_tiles); ++it_2) {
                uint64_t area = (it_1->x <= it_2->x) ? rectArea(*it_1, *it_2) : rectArea(*it_2, *it_1);
        ...
        while (!found && !squares_by_size.empty()) {
            ...
            std::cout << "x[" << x_min << ", " << x_max << "] y[" << y_min << ", " << y_max << "]" << std::endl;
            ...
            if (x_max > x_min + 1) {
                ...
                if (x_it_begin != x_it_end) {
                    ...
                    for (auto it = x_it_begin; it != x_it_end; ++it) {
                        ...
                        std::cout << "X line min/max: (" << tile_min.x << ", " << tile_min.y << ") (" << tile_max.x << ", " << tile_max.y << ")" << std::endl;
                        // When line is fully inside
                        if (tile_min.y >= y_min && tile_max.y <= y_max) goto bypass;
                        // When line is partially inside
                        if (tile_min.y > y_min && tile_min.y < y_max) goto bypass;
                        if (tile_max.y > y_min && tile_max.y < y_max) goto bypass;
                    }
                }
            }
            if (y_max > y_min + 1) {
                ...
                if (y_it_begin != y_it_end) {
                    ...
                    for (auto it = y_it_begin; it != y_it_end; ++it) {
                        auto & [tile_min, tile_max] = it->second;
                        std::cout << "Y line min/max: (" << tile_min.x << ", " << tile_min.y << ") (" << tile_max.x << ", " << tile_max.y << ")" << std::endl;
                        // When line is fully inside
                        if (tile_min.x >= x_min && tile_max.x <= x_max) goto bypass;
                        // When line is partially inside
                        if (tile_min.x > x_min && tile_min.x < x_max) goto bypass;
                        if (tile_max.x > x_min && tile_max.x < x_max) goto bypass;
                    }
                }
            }
            ...
            // If it's inside, any line starting from its centre will cross an odd number of red tiles, not an even number.
            {
                ...
                auto x_ref_begin = x_lines_set.lower_bound(x_max);
                ...
                if (lines_crossed % 2) goto bypass;
            }
            // Finally we are at the end. The rectangle is inside.
            std::cout << "Size: " << size << " (" << tile_1.x << ", " << tile_1.y << ") (" << tile_2.x << ", " << tile_2.y << ")" << std::endl;
...
private:
    static void drawBoard(const std::vector<Tile> & tiles) {
        std::size_t max_x = 0, max_y = 0;
        for (const auto & tile : tiles) {
            if (tile.x + 2 > max_x) max_x = tile.x + 2;
            if (tile.y + 2 > max_y) max_y = tile.y + 2;
        }

        std::vector board(max_y, std::string(max_x, '.'));
